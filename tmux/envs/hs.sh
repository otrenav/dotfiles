...
tmux send-keys -t $S:1 "cd ~/code/mll/hs/fe/" Enter
tmux_env_python $S 1

tmux new-window -t $S:2 -a -n shared
tmux send-keys -t $S:2 "cd ~/code/mll/hs/be-shr/" Enter
tmux_env_python $S 2

tmux new-window -t $S:3 -a -n svc-dvc-1
tmux send-keys -t $S:3 "cd ~/code/mll/hs/be-dvc/" Enter
tmux_env_python $S 3

tmux new-window -t $S:4 -a -n svc-dvc-2
tmux send-keys -t $S:4 "cd ~/code/mll/hs/be-dvc/" Enter
tmux_env_python $S 4

tmux new-window -t $S:5 -a -n svc-plt-1
tmux send-keys -t $S:5 "cd ~/code/mll/hs/be-plt/" Enter
tmux_env_python $S 5

tmux new-window -t $S:6 -a -n svc-plt-2
tmux send-keys -t $S:6 "cd ~/code/mll/hs/be-plt/" Enter
tmux_env_python $S 6
