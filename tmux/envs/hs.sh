...
tmux send-keys -t $S:1 "cd ~/code/mll/hs/plt/ehr/fe/" Enter
...
tmux send-keys -t $S:5 "cd ~/code/mll/hs/plt/be/" Enter
...
tmux new-window -t $S:8 -a -n ai-1
tmux send-keys -t $S:8 "cd ~/code/mll/hs/plt/" Enter
tmux send-keys -t $S:8 ". ./be/venv/bin/activate" Enter

tmux new-window -t $S:9 -a -n ai-2
