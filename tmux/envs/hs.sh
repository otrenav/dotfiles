...
tmux rename-window -t $S:1 inv-fe
tmux send-keys -t $S:1 "cd ~/code/mll/hs/plt/inv" Enter
tmux send-keys -t $S:1 "C-l" Enter
tmux send-keys -t $S:1 "git pull" Enter
tmux send-keys -t $S:1 ". ./start.sh" Enter

tmux new-window -t $S:2 -a -n sim-fe
tmux send-keys -t $S:2 "cd ~/code/mll/hs/plt/ehr/fe/" Enter
tmux send-keys -t $S:2 "C-l" Enter
tmux send-keys -t $S:2 ". ./start.sh" Enter

tmux new-window -t $S:3 -a -n sim-be
tmux send-keys -t $S:3 "cd ~/code/mll/hs/plt/ehr/be/" Enter
tmux send-keys -t $S:3 "C-l" Enter
tmux send-keys -t $S:3 ". ./start.sh" Enter

tmux new-window -t $S:4 -a -n mip-fe
tmux send-keys -t $S:4 "cd ~/code/mll/hs/plt/fe/" Enter
tmux send-keys -t $S:4 "C-l" Enter
tmux send-keys -t $S:4 "npm run dev:lcl" Enter

tmux new-window -t $S:5 -a -n mip-mod
tmux send-keys -t $S:5 "cd ~/code/mll/hs/mods/insights-models-mt-sinai-lvef-low/" Enter
